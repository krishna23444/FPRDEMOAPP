...
function [ret] = extract_finger_new(filename)
...
for i=1:minu_count
  ...
  for j = 1 : minu_count
    ...
    neighbours(n_i, :) = [i, ind(j),  dist_m(i,ind(j)), ridge_count, theta_t, minutiae(ind(j),3), minutiae(ind(j), 5), co, min(abs(minutiae(i, 4) - minutiae(ind(j), 4)),2*pi-abs(minutiae(i, 4) - minutiae(ind(j), 4))) ];
    n_i = n_i + 1;
  end
end
...
% % %   subplot(1,2,1), subimage(combined), title(filename)
% % %   subplot(1,2,2), subimage(thinned), title('thinned image')
...
% ret=struct('X', minutiae_for_sc, 'M', minutiae, 'O', orientations, 'R', radii, 'N', neighbours, 'RO', min_o_change, 'OIMG', orient_img_m, 'OREL', o_rel); %, 'SIKP', sikp_sc); 
ret = [ mean(minutiae_for_sc), mean(minutiae), mean(orientations), mean(radii), mean(neighbours), mean(min_o_change), mean(orient_img_m), mean(o_rel)] ;%, 'SIKP', sikp_sc); 
% ret = [ reshape(minutiae_for_sc,1,[]), reshape(minutiae,1,[]), reshape(orientations,1,[]), reshape(radii,1,[]), reshape(neighbours,1,[]), reshape(min_o_change,1,[]), reshape(orient_img_m,1,[]), reshape(o_rel,1,[])] ;%, 'SIKP', sikp_sc); 
