...
if isempty(mean_dist)
   tmp=r_array(in_vec,:);   %JI: removes rows not in in_vec
   tmp=tmp(:,in_vec);       %JI: removes cols not in in_vec
   mean_dist=mean(tmp(:));  %JI: global mean
end
...
gauss_dist =  repmat(exp( -((r_bin_edges - r_outer ).^2/2.5))', 1, nbins_theta);
gauss_dist = sort(gauss_dist(:)');
